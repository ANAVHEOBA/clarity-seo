...
class ReportService
{
    ...
    public function generate(Tenant $tenant, User $user, array $data): Report
    {
        ...
        if (! empty($data['template_id'])) {
            ...
            if ($template) {
                ...
                $data['branding'] = array_merge($template->branding ?? [], $data['branding'] ?? []);
                $data['filters'] = $data['filters'] ?? $template->filters;
            }
        }

        $data['branding'] = $this->resolveBranding($tenant, $data['branding'] ?? null);
     ...
     */
    private function resolveBranding(Tenant $tenant, ?array $branding): array
    {
        $branding = $branding ?? [];
        $defaultBranding = $tenant->brandingDefaults();

        $resolvedBranding = array_merge(
            Arr::except($defaultBranding, ['footer_text']),
            Arr::where($branding, static fn ($value) => $value !== null && $value !== '')
        );

        $whiteLabel = (bool) ($resolvedBranding['white_label'] ?? $tenant->isWhiteLabelEnabled());
        $resolvedBranding['white_label'] = $whiteLabel;

        if (empty($branding['footer_text'] ?? null)) {
            $resolvedBranding['footer_text'] = $whiteLabel
                ? ($tenant->support_email
                    ? 'Prepared by '.$tenant->brandDisplayName().' | '.$tenant->support_email
                    : 'Prepared by '.$tenant->brandDisplayName())
                : 'Generated by '.config('app.name');
        }

        return Arr::where($resolvedBranding, static fn ($value) => $value !== null && $value !== '');
    }
